...
This bot uses **[TOON (Token-Oriented Object Notation)](https://github.com/toon-format/toon)** to compress large context feeds into a compact, model-readable format.
- **Reference**: `toon-format` is used via `npx @toon-format/cli`.
- **Why**: Drastically reduces token usage (up to 40% vs JSON) while maintaining structure for the LLM.
- **Enable**: `use_toon_cli: true` in config (on by default). Requires `npx`.

- Personas can also be defined in `.toon` format for extra efficiency.
